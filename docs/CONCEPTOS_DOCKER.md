...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8066"]
    ...
    FRONTFLASK -->|"http://api-facturas:8065<br/>al controlador, por el NOMBRE"| APIFACTURAS
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13353"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15459"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11459"| SQLSERVER
```
...
```yaml
URL_API: http://api-facturas:8065
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8065` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8065` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8065:8065`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8065`, el front
...
```powershell
docker network inspect proyecto_diseno_de_software8_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_diseno_de_software8` produce
la red `proyecto_diseno_de_software8_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_diseno_de_software8-front-flask-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_diseno_de_software8_default has active endpoints (name:"proyecto_diseno_de_software8-front-flask-1" …)
```
